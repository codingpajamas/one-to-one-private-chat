...
<script>
    ...
    function sendMessage() {
        ...
        $.post('http://localhost/chat-pusher-real-time-app/public/chat/message', data).success(sendMessageSuccess);
    ...
    var pusher = new Pusher('{{env("PUSHER_KEY")}}', {
                    cluster: 'ap1',
                    authEndpoint: 'http://localhost/chat-pusher-real-time-app/public/chat/auth',
                    auth: {
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        }
