...
<script>
    ...
    function statusUpdateSubmit() {
        ...
        $.post('http://localhost/chat-pusher-real-time-app/public/activities/status', data).success(statusUpdateSuccess);
    ...
    function sendLike(id) {
        $.post('http://localhost/chat-pusher-real-time-app/public/activities/like/' + id).success(likeSuccess);
    }
    ...
    channel.bind('status-update-liked', function(data) {
        ...
        // add like count
        var elStatusCount = $('[data-activity-id="'+data.likedActivityId+'"]').find('.like-count'); 
        var strStatusLikeCount = elStatusCount.text() == '' ? 1 : parseInt(elStatusCount.text())+1;
        elStatusCount.text(strStatusLikeCount);
        console.log(elStatusCount, strStatusLikeCount, elStatusCount.text())
    });
</script>
